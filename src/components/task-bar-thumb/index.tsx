...
const TaskBarThumb: React.FC<TaskBarProps> = ({ data }) => {
  ...
  return (
    ...
    >
      {
        type === 'left' && <div className={`${prefixClsTaskBarThumb}-circle-left`} style={{ backgroundColor:getBackgroundColor }} />
      }
      {renderBarThumb ? renderBarThumb(data.record, type) : label}
      {
        type === 'right' && <div className={`${prefixClsTaskBarThumb}-circle-right`} style={{ backgroundColor:getBackgroundColor }} />
      }
    </div>
  );
};
